...
class Menu(ttk.Frame):
    ...
    def styles(self):
        self.style.configure('TButton', **self.fonts)

    def startMenu(self):
        self.image, (w,h) = openPic("adamas.png")
        self.mainPic = ttk.Label(self, width=w, image=self.image)
        self.mainPic.pack(anchor='center', **self.options)
    ...
    def SetDialog(self):
        ...
        self.win.grab_set()
        self.win.geometry(gameSet['geometry_settings'])
    ...
    def change_frame(self, name):
        ...
        self.frames = {'gameScreen' : GameScreen(self.container, self.options, self.fonts)}
...
class GameScreen(ttk.Frame):
    def __init__(self, container, options, fonts={'font': ('Helvetica',15)}):
        super().__init__(container)
        self.container = container
        self.options = options
        self.fonts = fonts
    ...
    def styles(self):
        self.style.configure('TButton', **self.fonts)
        self.style.configure('TEntry', **self.fonts)
        self.style.configure('TLabel', **self.fonts)
    ...
    def startGame(self):
        ...
        self.temp, (w,h) = openPic('adamas.png',size=(300,300))
        ...
        self.StateIm = ttk.Label(self, width=w, image = self.temp)
        self.StateIm.grid(row=1, column=1, **self.options)
...
#------------------------------------------------------------------------------------------------
    ...
    def change_frame(self):
        frame = Menu(self.container, self.options, self.fonts)
        self.destroy()
        frame.tkraise()
    ...
    def changePic(self):
        self.im ,(w,h)= openPic(f'state{self.rights}.png')
        self.StateIm.configure(image=self.im)
...
class SetScreen(ttk.Frame):
    def __init__(self, container, options, fonts):
        super().__init__(container)
